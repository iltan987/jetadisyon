...
CREATE POLICY policy_tenants_all_admin ON public.tenants
  FOR ALL USING (((jwt() -> 'app_metadata'::text) ->> 'user_role'::text) = 'admin');
...
CREATE POLICY policy_audit_logs_select_admin ON public.audit_logs
  FOR SELECT USING (((jwt() -> 'app_metadata'::text) ->> 'user_role'::text) = 'admin');
...
-- 8. Grant execute to supabase_auth_admin (required for auth hooks)
