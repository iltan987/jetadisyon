...
CREATE POLICY policy_tenants_select ON public.tenants
  FOR SELECT USING (
    ((select auth.jwt()) -> 'app_metadata'::text ->> 'system_role'::text) = 'admin'
...
CREATE POLICY policy_tenants_insert_admin ON public.tenants
  FOR INSERT WITH CHECK (((select auth.jwt()) -> 'app_metadata'::text ->> 'system_role'::text) = 'admin');

CREATE POLICY policy_tenants_update_admin ON public.tenants
  FOR UPDATE USING (((select auth.jwt()) -> 'app_metadata'::text ->> 'system_role'::text) = 'admin');

CREATE POLICY policy_tenants_delete_admin ON public.tenants
  FOR DELETE USING (((select auth.jwt()) -> 'app_metadata'::text ->> 'system_role'::text) = 'admin');
...
CREATE POLICY policy_audit_logs_select_admin ON public.audit_logs
  FOR SELECT USING (((select auth.jwt()) -> 'app_metadata'::text ->> 'system_role'::text) = 'admin');

-- 10. Custom Access Token Hook
-- Injects system_role, tenant_role, and tenant_id into JWT claims
